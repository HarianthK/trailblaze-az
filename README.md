...
Live at **[trailblaze-az.vercel.app](https://trailblaze-az.vercel.app)**.
...
both ways at once: the one you chose in colour, the other as a dashed line. You
see the difference rather than having to remember it. The ground is shaded, so
the mountains the long way climbs over are visible rather than implied.
...
**And it treats the trip as a day out.** Nobody driving to Sedona on a Sunday
is moving house, so the app shows the loop: out the pretty way, back the quick
one. Six hours ten of driving, leave at eight, home by four. It also says when
that conflicts with the previous point, because staying for golden hour means
driving the mountain road home in the dark.
...
**A chosen trip has its own address**, so you can send someone the drive rather
...
Gilbert has no scenic alternative to find. It was once the headline example in
this README; the data says it is the *weakest* case for the idea, and long rural
drives are where it earns its keep.

### If you did want a routing server

You do not need one, and this project no longer has one. But
[`routing/scenic.lua`](routing/scenic.lua) is still here, and it is the same
weighting the browser applies, expressed as an OSRM profile. Anyone with a
machine that can host OSRM can point it at the tagged extract from stage 6 and
get the same routes from a server instead.

That was the original plan and it is written up under "Why OSRM and not
Valhalla" above, because the reasoning still holds even though the server does
not exist. What killed it was hardware rather than design: building the graph
needs Docker and several free gigabytes, and the machine this was written on has
neither.
...
option has to infer from `highway=track` and friends rather than trust the
tag, treating untagged as paved would route people onto dirt.
